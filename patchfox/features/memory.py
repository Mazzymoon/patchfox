...
def write_dream_report(memory_dir, report, iso_ts=None):
    ...
    filename_ts = re.sub(r'[:<>"/\\|?*]', "-", str(iso_ts))
    path = reports_dir / f"{filename_ts}.json"
...
You are performing a dream: a reflective pass over PatchFox's memory files. Synthesize recent signal into durable, well-organized memory files so future sessions can orient quickly.
...
def run_dream(agent, quiet=False, session_ids=None):
    ...
    try:
        ...
        memory_scope = Path(".patchfox") / "memory"
    dream_agent = PatchFox(
        ...
        auto_dream=False,
        shell_backend=agent.shell_backend,
    )
...
def maintain_memory_after_turn(agent, final_answer):
    ...
    thread = threading.Thread(target=_background_dream, name="patchfox-auto-dream", daemon=True)
...
def workspace_fingerprint(workspace_root):
    ...
    try:
        ...
        git_root = str(Path(git_root_output).resolve())
        fingerprint = hashlib.sha256(git_root.encode("utf-8")).hexdigest()[:12]
    except Exception:
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".patchfox" / "memory" if workspace_root is not None else None
...
def _iter_retrieval_notes(state, workspace_root=None):
    for note in state["episodic_notes"]:
        yield dict(note)
    if workspace_root is not None:
        durable_store = DurableMemoryStore(Path(workspace_root) / ".patchfox" / "memory")
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(Path(workspace_root) / ".patchfox" / "memory") if workspace_root is not None else None
