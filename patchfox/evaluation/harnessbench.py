...
PatchFox already persists run trace, report, task state, session JSON, and session
events under `.patchfox/`. Harness-Bench adapters need a stable manifest that points
...
def build_adapter_metadata(
    ...
    *,
    ...
    latest_run = _latest_dir(workspace / ".patchfox" / "runs")
    sessions_root = workspace / ".patchfox" / "sessions"
    ...
    metadata: dict[str, Any] = {
        ...
        "patchfox_evidence_available": bool(latest_run),
        "patchfox_evidence_missing": [],
    }
    _add_path(metadata, "patchfox_run_dir", latest_run)
    if latest_run:
        _add_path(metadata, "patchfox_trace_path", latest_run / "trace.jsonl")
        _add_path(metadata, "patchfox_report_path", latest_run / "report.json")
        _add_path(metadata, "patchfox_task_state_path", latest_run / "task_state.json")
    _add_path(metadata, "patchfox_session_path", session_path)
    _add_path(metadata, "patchfox_session_event_path", event_path)
    transcript_path = _write_process_transcript(session_path)
    _add_path(metadata, "patchfox_process_transcript_path", transcript_path)

    required = (
        "patchfox_run_dir",
        "patchfox_trace_path",
        "patchfox_report_path",
        "patchfox_task_state_path",
        "patchfox_session_path",
        "patchfox_session_event_path",
        "patchfox_process_transcript_path",
    )
    metadata["patchfox_evidence_missing"] = [
        key for key in required if not metadata.get(key)
    ]
    metadata["patchfox_evidence_available"] = not metadata["patchfox_evidence_missing"]
    return metadata
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Emit PatchFox Harness-Bench adapter metadata.")
