"""Read-only helpers for PatchFox run/session evidence.

These helpers intentionally do not import or instantiate the PatchFox runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.patchfox/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".patchfox" / "runs")
        ...
        session_path = _latest_file(workspace / ".patchfox" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".patchfox" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
