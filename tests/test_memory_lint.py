...
from patchfox.features.memory_lint import lint_memory_dir
...
def test_memory_lint_cli_returns_one_for_findings():
    result = subprocess.run(
        [sys.executable, "-m", "patchfox.features.memory_lint", "tests/fixtures/memory_lint_dirty"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_memory_lint_duplicate_active_subject_positive_and_negative(tmp_path):
    _write_memory_fixture(
        tmp_path,
        ["PatchFox uses pytest.", "PatchFox uses unittest."],
        [_row("PatchFox uses pytest."), _row("PatchFox uses unittest.", status="superseded")],
    )
    ...
    _write_memory_fixture(
        tmp_path / "dirty",
        ["PatchFox uses pytest.", "PatchFox uses unittest."],
        [_row("PatchFox uses pytest."), _row("PatchFox uses unittest.")],
    )
...
def test_memory_lint_orphan_supersede_positive_and_negative(tmp_path):
    _write_memory_fixture(tmp_path, ["PatchFox uses pytest."], [_row("PatchFox uses pytest.", supersedes=None)])
    assert not [finding for finding in lint_memory_dir(tmp_path) if finding["rule"] == "orphan_supersede"]

    _write_memory_fixture(tmp_path / "dirty", ["PatchFox uses pytest."], [_row("PatchFox uses pytest.", supersedes="missing")])
...
def test_memory_lint_missing_evidence_positive_and_negative(tmp_path):
    _write_memory_fixture(tmp_path, ["PatchFox should keep evidence."], [_row("PatchFox should keep evidence.")])
    assert not [finding for finding in lint_memory_dir(tmp_path) if finding["rule"] == "missing_evidence"]

    _write_memory_fixture(tmp_path / "dirty", ["PatchFox should keep evidence."], [_row("PatchFox should keep evidence.", session_id="")])
