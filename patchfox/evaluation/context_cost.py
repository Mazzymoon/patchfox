...
from patchfox.config import default_max_tokens_for_provider, resolve_provider_config
from patchfox.core.run_store import RunStore
from patchfox.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient
...
def _build_synthetic_agent(workspace_root, *, context_reduction=True):
    ...
    agent = PatchFox(
        ...
        session_store=SessionStore(workspace_root / ".patchfox" / "sessions"),
...
def _run_long_session_task(
    ...
    *,
    ...
    variant_dir = {
        "full_orchestrator": "full",
        "full_orchestrator_with_llm_handoff": "llm-handoff",
    }.get(variant, variant)
    workspace = (
        Path(output_dir)
        / "runs"
    ...
    agent = PatchFox(
        ...
        run_store=RunStore(workspace / ".patchfox" / "runs"),
        approval_policy="auto",
        feature_flags={
...
def _build_arg_parser():
    parser = argparse.ArgumentParser(description="Run PatchFox context cost experiments.")
