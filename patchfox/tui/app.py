...
from .provider_picker import ProviderModelPicker
from .widgets import (
    AskUserPrompt,
    ChatLog,
    ConfirmPrompt,
...
class PatchFoxTuiApp(App):
    """Textual shell for the existing PatchFox runtime.

    The TUI is deliberately a presentation layer: CLI argument parsing and agent
    construction still live in `patchfox.cli`, while turns are driven through the
    same `Engine.run_turn()` generator that powers the plain REPL.
    """

    CSS = PATCHFOX_TUI_CSS
    BINDINGS = [
    ...
    def _handle_command(self, text: str) -> None:
        if text in {"/provider", "/model"} and callable(
            getattr(self.agent, "switch_provider", None)
        ):
            self.push_screen(
                ProviderModelPicker(
                    getattr(self.agent, "provider_name", ""),
    ...
    def _apply_provider_selection(self, selection) -> None:
        if not selection:
            self.query_one(InputBar).focus_input()
            return
        provider, model = selection
        try:
            config = self.agent.switch_provider(provider, model)
            output = f"provider: {config.name}\nmodel: {config.model}"
