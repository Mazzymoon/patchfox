...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".patchfox" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".patchfox").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".patchfox" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "patchfox/0.4.0"
...
def test_openai_compatible_client_retries_rate_limit_and_records_retry_metadata():
    ...
    with patch("urllib.request.urlopen", fake_urlopen), patch("patchfox.providers.clients.time.sleep"):
        result = client.complete("hello", 42)
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("patchfox.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = patchfox_pkg.build_agent(args)
...
def test_build_arg_parser_reports_patchfox_version(capsys):
    with pytest.raises(SystemExit) as exc_info:
        patchfox_pkg.build_arg_parser().parse_args(["--version"])

    assert exc_info.value.code == 0
    assert capsys.readouterr().out.strip() == "PatchFox 0.4.0"


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = patchfox_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        ), patch("patchfox.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("patchfox.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            patchfox_pkg.build_agent(args)
...
def test_build_agent_does_not_load_provider_credentials_from_project_env(
    ...
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "PATCHFOX_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "PATCHFOX_DEEPSEEK_API_KEY=sk-project-deepseek",
                "PATCHFOX_DEEPSEEK_MODEL=deepseek-v4-pro",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert "PATCHFOX_DEEPSEEK_API_KEY" not in os.environ
    assert agent.model_client is fake_client
...
def test_project_toml_cannot_override_provider_endpoint_or_process_credential(
    ...
    with patch.dict(os.environ, {"PATCHFOX_DEEPSEEK_API_KEY": "sk-legacy-env"}, clear=True):
        with patch(
            "patchfox.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("patchfox.cli.AnthropicCompatibleModelClient") as mock_anthropic:
    ...
    assert mock_anthropic.call_args.kwargs["model"] == "deepseek-v4-pro"
    assert mock_anthropic.call_args.kwargs["base_url"] == "https://api.deepseek.com/anthropic"
    assert mock_anthropic.call_args.kwargs["api_key"] == "sk-legacy-env"
    assert agent.provider_warnings
    assert agent.model_client is fake_client
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".patchfox" / "runs"
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .patchfox/.\n"
    ...
    index_path = tmp_path / ".patchfox" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".patchfox" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".patchfox" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .patchfox/.",
...
def test_final_memory_tags_are_appended_to_daily_log(tmp_path):
    ...
    log_files = list((tmp_path / ".patchfox" / "memory" / "logs").rglob("*.md"))
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["memory_maintenance"]["memory_tags_appended"][0]["path"].startswith(".patchfox/memory/logs/")
...
def test_auto_dream_runs_in_background_after_session_gate(tmp_path):
    for index in range(2):
        (tmp_path / ".patchfox" / "sessions" / f"older-{index}.json").parent.mkdir(parents=True, exist_ok=True)
        (tmp_path / ".patchfox" / "sessions" / f"older-{index}.json").write_text("{}", encoding="utf-8")
    agent = build_agent(
        tmp_path,
        [
            "<final><memory>Project: use repo-local memory.</memory></final>",
            '<tool>{"name":"read_file","args":{"path":".patchfox/memory/MEMORY.md","start":1,"end":50}}</tool>',
            '<tool>{"name":"write_file","args":{"path":".patchfox/memory/MEMORY.md","content":"# Durable Memory Index\\n\\n- [Project](topics/project.md): Project memory\\n"}}</tool>',
            "<final>Dreamed.</final>",
        ],
...
def test_background_auto_dream_failure_restores_lock_and_reports_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("patchfox.features.memory.run_dream", fail_dream)
    agent = build_agent(
        tmp_path,
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".patchfox" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".patchfox" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert not hasattr(patchfox_pkg, "ScriptedModelClient")
    assert not hasattr(providers_pkg, "ScriptedModelClient")
    assert PatchFox is not None
    assert not hasattr(patchfox_pkg, "OllamaModelClient")
    assert SessionStore is not None
    assert WorkspaceContext is not None
    assert Path(patchfox_pkg.__file__).as_posix().endswith("/patchfox/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(patchfox_pkg.main)
    assert callable(patchfox_pkg.build_agent)
    assert callable(patchfox_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "patchfox", "--help"],
        capture_output=True,
        text=True,
    )
