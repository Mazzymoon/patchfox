...
import patchfox.cli as patchfox_cli
from patchfox.core.runtime import PatchFox
from patchfox.core.model_router import ModelClientRouter
from patchfox.core.session_store import SessionStore
from patchfox.core.workspace import WorkspaceContext
from patchfox.providers.clients import AnthropicCompatibleModelClient, OpenAICompatibleModelClient
...
def test_openai_client_sends_image_blocks_in_responses_payload(monkeypatch):
    from patchfox.core.content_blocks import ImageBlock, ModelInput
...
def test_vision_provider_uses_vision_specific_env_overrides(tmp_path, monkeypatch):
    from patchfox.config import resolve_vision_provider_config

    monkeypatch.setenv("PATCHFOX_OPENAI_API_KEY", "sk-text")
    monkeypatch.setenv("PATCHFOX_OPENAI_MODEL", "text-model")
    monkeypatch.setenv("PATCHFOX_OPENAI_API_BASE", "https://text.example/v1")
    monkeypatch.setenv("PATCHFOX_VISION_API_KEY", "sk-vision")
    monkeypatch.setenv("PATCHFOX_VISION_MODEL", "vision-model")
    monkeypatch.setenv("PATCHFOX_VISION_API_BASE", "https://vision.example/v1")
...
def test_build_agent_uses_separate_vision_provider_for_deepseek(tmp_path, monkeypatch):
    ...
    with pytest.MonkeyPatch.context() as patcher:
        patcher.setattr(patchfox_cli, "AnthropicCompatibleModelClient", lambda **kwargs: ("anthropic", kwargs))
        patcher.setattr(patchfox_cli, "OpenAICompatibleModelClient", lambda **kwargs: ("openai", kwargs))
        agent = patchfox_cli.build_agent(args)
...
def test_inspect_image_uses_separate_vision_model_when_configured(tmp_path):
    from patchfox.core.content_blocks import ModelInput
...
def test_image_inspection_prompt_preserves_complete_ocr_extraction():
    from patchfox.core.vision import image_inspection_prompt
...
def test_vision_model_call_has_total_timeout(monkeypatch):
    from patchfox.core.content_blocks import ModelInput
    from patchfox.core.vision import complete_model_with_timeout
    ...
    monkeypatch.setattr("patchfox.core.vision.complete_model", slow_complete)
...
def test_load_workspace_image_rejects_path_escape_and_records_safe_metadata(tmp_path):
    from patchfox.core.media import load_workspace_image
