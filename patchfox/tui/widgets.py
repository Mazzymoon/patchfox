...
class WelcomeBanner(Static):
    ...
    def render(self) -> Text:
        ...
        rows = [
            Text.assemble(
                Text("patchfox", style=f"bold {accent}"),
                Text("  local coding agent", style=muted),
            ),
            Text(""),
        ]
        rows.extend(Text(line, style=accent) for line in PATCHFOX_MARK)
        rows.extend(
            [
                Text(""),
                Text.assemble(
...
class AssistantMessage(Static):
    ...
    def compose(self):
        yield Static("patchfox", classes="message-label")
...
class StatusBar(Static):
    ...
    def update_agent(self, agent) -> None:
        ...
        session = str(agent.session.get("id", ""))[-6:]
        identity = (
            f"{provider}/{model}"
            if provider and model
            else provider or model or "-"
        )
        parts = [identity]
        if mode != "default":
            parts.append(mode)
        parts.append(f"s:{session or '-'}")
        self.agent_text = " | ".join(parts)
        self._render_status()
    ...
    def update_context_usage(self, usage: dict | None) -> None:
        ...
        approximate = source in {"estimated", "char_estimate"}
        marker = "~" if approximate else ""
        if used and budget:
            self.context_text = (
                f"ctx:{marker}{_compact_status_number(used)}/"
                f"{_compact_status_number(budget)}"
            )
        elif used:
            self.context_text = f"ctx:{marker}{_compact_status_number(used)}"
        else:
            self.context_text = "ctx:-"
        extras = []
        pressure = str(usage.get("pressure_tier", "") or "")
        if pressure and pressure not in {"tier0_observe", "observe", "low"}:
            pressure = pressure.removeprefix("tier").lstrip("0123456789_-")
            extras.append(f"pressure:{pressure or usage['pressure_tier']}")
        if extras:
    ...
    def _render_status(self) -> None:
        self.update(
            f"{self.agent_text} | t:{self.turns} | {self.context_text}".strip()
        )


def _compact_status_number(value) -> str:
    number = int(value)
...
class InputBar(Static):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.input = Input(placeholder="Ask patchfox or type /help")
    ...
    def set_busy(self, busy: bool) -> None:
        ...
        self.input.placeholder = (
            "patchfox is working..." if busy else "Ask patchfox or type /help"
        )
