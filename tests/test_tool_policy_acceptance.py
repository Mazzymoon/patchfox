...
def test_patch_requires_prior_fresh_read_and_allows_after_read(tmp_path):
    ...
    rejected = agent.run_tool("patch_file", {"path": "README.md", "old_text": "world", "new_text": "patchfox"})
    ...
    patched = agent.run_tool("patch_file", {"path": "README.md", "old_text": "world", "new_text": "patchfox"})

    assert patched == "patched README.md"
    assert (tmp_path / "README.md").read_text(encoding="utf-8") == "hello patchfox\n"
...
def test_rejected_patch_can_be_retried_after_informing_read(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            '<tool name="patch_file" path="README.md"><old_text>world</old_text><new_text>patchfox</new_text></tool>',
...
def test_tool_governance_decisions_are_run_trace_evidence(tmp_path):
    ...
    assert report["evidence_summaries"]["governance_summary"] == {
        "schema_version": "patchfox.governance_summary.v1",
        "allow_count": 1,
        "deny_count": 2,
        "warn_count": 0,
