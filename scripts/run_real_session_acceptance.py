#!/usr/bin/env python3
"""Run deterministic real-session acceptance scenarios for PatchFox."""
...
from patchfox import PatchFox, SessionStore, WorkspaceContext  # noqa: E402
from patchfox.config import resolve_provider_config  # noqa: E402
from patchfox.features.skills_runtime import invoke_skill  # noqa: E402
from patchfox.providers import AnthropicCompatibleModelClient, OpenAICompatibleModelClient, ProviderError  # noqa: E402
from patchfox.testing import ScriptedModelClient  # noqa: E402

SUMMARY_JSON = "gate8-real-session-acceptance.json"
SUMMARY_MARKDOWN = "gate8-real-session-acceptance.md"
LIVE_ENV_FLAG = "PATCHFOX_ACCEPTANCE_LIVE"
...
def _run_scenario(output_dir, scenario_id, runner, optional=False, include_live=None):
    ...
    try:
        ...
        return record
    except Exception as exc:  # noqa: BLE001 - acceptance must preserve scenario evidence
        return {
            "id": scenario_id,
            "status": "failed",
...
def _scenario_bugfix_pytest(output_dir, workspace):
    ...
    pytest_command = f'"{Path(sys.executable).as_posix()}" -m pytest -q'
    agent = _build_agent(
        workspace,
        [
            ...
            + json.dumps(
                {
                    "name": "run_shell",
                    "args": {"command": pytest_command, "timeout": 60},
                }
            )
            + "</tool>",
...
def _scenario_plan_todo_explore(output_dir, workspace):
    ...
    agent = _build_agent(
        ...
        [
            ...
            '<tool name="write_file" path=".patchfox/plans/gate8-plan.md"><content># Gate8 Plan\n- Evidence harness\n</content></tool>',
    ...
    return _finalize(
        ...
        checks=[
            ...
            _check("plan_file", (workspace / ".patchfox" / "plans" / "gate8-plan.md").is_file()),
...
def _scenario_skill_inline(output_dir, workspace):
    ...
    skill_dir = workspace / ".patchfox" / "skills" / "evidence"
...
def _scenario_resume_continuation(output_dir, workspace):
    ...
    store = SessionStore(workspace / ".patchfox" / "sessions")
    first = PatchFox(
...
def _scenario_security_rejection(output_dir, workspace):
    ...
    old_secret = os.environ.get("PATCHFOX_ACCEPTANCE_SECRET")
    os.environ["PATCHFOX_ACCEPTANCE_SECRET"] = "patchfox-secret-value-123"
    agent = _build_agent(
        workspace,
        [
            ...
            '<tool>{"name":"run_shell","args":{"command":"echo $PATCHFOX_ACCEPTANCE_SECRET","timeout":5}}</tool>',
    ...
    try:
        ...
        return _finalize(
            ...
            checks=[
                ...
                _check("secret_redacted", "patchfox-secret-value-123" not in trace_text),
            ],
        )
    finally:
        if old_secret is None:
            os.environ.pop("PATCHFOX_ACCEPTANCE_SECRET", None)
        else:
            os.environ["PATCHFOX_ACCEPTANCE_SECRET"] = old_secret
...
def _scenario_provider_error_recovery(output_dir, workspace):
    ...
    agent = PatchFox(
        ...
        workspace=_scenario_workspace(workspace),
        session_store=SessionStore(workspace / ".patchfox" / "sessions"),
        approval_policy="auto",
        max_steps=2,
    )
...
def _scenario_live_provider_smoke(output_dir, workspace, include_live=None):
    ...
    agent = PatchFox(
        ...
        max_steps=1,
        max_new_tokens=64,
        secret_env_names=["PATCHFOX_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DEEPSEEK_API_KEY"],
    )
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Run PatchFox Gate8 deterministic real-session acceptance scenarios.")
