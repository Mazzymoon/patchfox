...
from patchfox import cli as patchfox_cli
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = patchfox_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = patchfox_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["GH_PAT"]


def test_cli_build_agent_does_not_load_project_env_secrets(tmp_path):
    class DummyModelClient:
        def __init__(self, *args, **kwargs):
            self.args = args
            self.kwargs = kwargs
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("PATCHFOX_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("patchfox.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = patchfox_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = patchfox_cli.build_agent(args)
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "PATCHFOX_CUSTOM_SECRET": "custom-secret-value",
            "PATCHFOX_SECRET_ENV_NAMES": "PATCHFOX_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("patchfox.cli.OpenAICompatibleModelClient", DummyModelClient):
        args = patchfox_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = patchfox_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["PATCHFOX_CUSTOM_SECRET"]
...
def test_bound_tool_methods_call_tools_module(tmp_path):
    ...
    with patch.object(agent.shell_backend, "run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "patchfox.core.runtime"
