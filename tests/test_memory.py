...
def test_durable_memory_index_and_topic_notes_are_loaded_and_retrieved(tmp_path):
    ...
    (topics_dir / "project-conventions.md").write_text(
        ...
        "- Preserve local agent state under .patchfox/.\n",
        encoding="utf-8",
    )
...
def test_structured_durable_promote_records_supersede_metadata(tmp_path):
    ...
    promoted, superseded = memory.promote_durable(
        [
            ("project-conventions", "PatchFox uses unittest."),
            ("project-conventions", "PatchFox uses pytest."),
        ]
    )
    ...
    assert superseded == ["project-conventions: PatchFox uses unittest. -> PatchFox uses pytest."]
    rows = [
        json.loads(line)
        for line in (tmp_path / ".patchfox" / "memory" / "topics" / "project-conventions.metadata.jsonl")
...
def test_stale_evidence_rejects_durable_note_when_anchor_changes(tmp_path):
    ...
    metadata_path = tmp_path / ".patchfox" / "memory" / "topics" / "project-conventions.metadata.jsonl"
    rows = [json.loads(line) for line in metadata_path.read_text(encoding="utf-8").splitlines()]
...
def test_dream_prompt_targets_repo_local_memory_assets(tmp_path):
    ...
    prompt = build_dream_prompt(memory_root, transcript_dir=str(tmp_path / ".patchfox" / "sessions"), session_ids=["s1", "s2"])
...
def test_dream_prompt_uses_four_phase_filesystem_maintenance_flow(tmp_path):
    memory_root = tmp_path / ".patchfox" / "memory"
    transcript_dir = tmp_path / ".patchfox" / "sessions"
...
def test_dream_writes_quality_report_under_memory_dir(tmp_path):
    agent = build_runtime_agent(
        tmp_path,
        [
            '<tool>{"name":"write_file","args":{"path":".patchfox/memory/topics/test-topic.md","content":"# Test Topic\\n\\n## Notes\\n- PatchFox keeps stable signal.\\n"}}</tool>',
    ...
    report = latest_dream_report(tmp_path / ".patchfox" / "memory")
...
def test_session_scan_deduplicates_session_files_and_event_logs(tmp_path):
    sessions_dir = tmp_path / ".patchfox" / "sessions"
