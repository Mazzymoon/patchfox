...
from .. import PatchFox, SessionStore, WorkspaceContext
...
CONSOLIDATED_NOTES = {
    "01_clean_signal": [
        "PatchFox benchmark reports must separate deterministic and live-provider evidence.",
        "Memory evaluation artifacts live under _local/benchmark/artifacts.",
        "Roadmap PRs should keep unrelated refactors out.",
    ],
    "02_noise_heavy": [
        "PatchFox memory fidelity evaluates irrelevant injection, supersede, secret, stale, and poison cases.",
    ],
    "03_secret_leak": [],
    "04_duplicates": [
        "PatchFox uses repo-local memory snapshots for deterministic evaluation.",
    ],
    "05_relative_dates": [
    ...
    "06_mixed": [
        "PatchFox should reject poisoned durable notes before retrieval.",
        "PatchFox uses repo-local memory snapshots for deterministic evaluation.",
        "The release review happens on 2026-06-02.",
    ],
}
...
def _scripted_outputs(fixture_name):
    ...
    return [
        '<tool>{"name":"read_file","args":{"path":".patchfox/memory/topics/test-topic.md","start":1,"end":120}}</tool>',
        f'<tool>{{"name":"write_file","args":{{"path":".patchfox/memory/topics/test-topic.md","content":{content}}}}}</tool>',
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Run PatchFox dream quality fixtures.")
