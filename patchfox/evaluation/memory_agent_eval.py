...
DEFAULT_REPORT_PATH = Path("docs/metrics/patchfox-memory-evaluation-report.md")
...
def _stale_evidence_case():
    with tempfile.TemporaryDirectory(prefix="patchfox-memory-agent-stale-") as temp_dir:
        ...
        metadata_path = workspace_root / ".patchfox" / "memory" / "topics" / "project-conventions.metadata.jsonl"
...
def _build_challenge_cases():
    return [
        ...
        _case(
            ...
            "information_extraction",
            "What storage constraint applies to PatchFox memory?",
            "filesystem-first; no vector DB",
            [
                _mem("mem-vector-db-distractor", "PatchFox memory uses a vector database.", "vector database", ("memory", "storage"), "2026-06-24T10:02:00+00:00", "superseded"),
                _mem("mem-filesystem-memory", "PatchFox memory is filesystem-first and must not use vector DB.", "filesystem-first; no vector DB", ("memory", "storage"), "2026-06-24T10:01:00+00:00"),
            ],
        ...
        _case(
            ...
            [
                _mem("mem-provider-free", "PatchFox memory benchmark uses provider-free deterministic fixtures.", "provider-free deterministic", ("provider", "benchmark"), source="session-a"),
        ...
        _case(
            ...
            "No.",
            [
                _mem("mem-prod-reliability-old", "PatchFox proves production long-term memory reliability.", "Yes.", ("claim",), "2026-06-24T10:03:00+00:00", "superseded"),
                _mem("mem-prod-reliability-new", "PatchFox proves deterministic coding-agent memory contracts and challenge results.", "No.", ("claim",), "2026-06-24T10:01:00+00:00"),
            ],
        ...
        _case(
            ...
            "abstention",
            "What vector DB does PatchFox memory use?",
            "unknown",
        ...
        _case(
            ...
            "docs/metrics/patchfox-benchmark-core-report.md",
            [_mem("mem-known-file", "Already inspected docs/metrics/patchfox-benchmark-core-report.md for memory metrics.", "docs/metrics/patchfox-benchmark-core-report.md", ("file", "metrics"))],
            ["mem-known-file"],
        ...
        _case(
            ...
            "information_extraction",
            "What tool-call format does PatchFox expect?",
        ...
        _case(
            ...
            "abstention",
            "What cloud vendor hosts PatchFox production?",
            "unknown",
...
def render_memory_evaluation_report(artifact):
    ...
    lines = [
        "# PatchFox 长期记忆系统与 Challenge Benchmark 报告",
        "",
        "## Resume Claim",
        "",
        "> 设计并实现 PatchFox 面向 Coding Agent 的长期记忆系统，覆盖结构化检索、事实更新、过期证据拒用、敏感信息隔离与跨会话任务恢复；参考 LongMemEval/BEAM 设计 provider-free memory challenge benchmark，引入 memory-off、naive-recent、unsafe-memory baseline，对比验证 evidence recall@k、stale-use、secret exposure、abstention、false-resume 与重复上下文读取等指标。",
        "",
        "## What Was Built",
        "",
        "- 文件优先 durable memory：长期事实落在 `.patchfox/memory/topics/*.md`，主存储保持可读、可 diff、可审计。",
