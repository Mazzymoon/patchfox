...
这个模块负责把“用户怎么启动 patchfox”翻译成 runtime 能理解的对象：
...
from .config import (
    ...
    resolve_project_shell_config,
    resolve_project_sandbox_config,
)
from .config.setup import (
    configuration_missing,
    confirm_and_migrate_legacy,
    render_config_show,
    run_text_setup,
)
from .config.store import load_global_config, save_project_selection
...
from .core.runtime import PatchFox, SessionStore
...
DEFAULT_SECRET_ENV_NAMES = (
    ...
    "OPENAI_API_KEY",
    "OPENAI_API_TOKEN",
    "PATCHFOX_VISION_API_KEY",
    "PATCHFOX_ANTHROPIC_API_KEY",
    "ANTHROPIC_API_KEY",
    "ANTHROPIC_AUTH_TOKEN",
    "PATCHFOX_DEEPSEEK_API_KEY",
    "DEEPSEEK_API_KEY",
...
SECRET_ENV_NAMES_VAR = "PATCHFOX_SECRET_ENV_NAMES"
...
def build_agent(args):
    """根据 CLI 参数装配出一个可运行的 PatchFox 实例。
    ...
    - 输出：一个新的 `PatchFox`，或一个从旧 session 恢复出来的 `PatchFox`
    ...
    setattr(args, "_project_root", workspace.repo_root)
    if not hasattr(args, "_requested_max_new_tokens"):
        setattr(args, "_requested_max_new_tokens", getattr(args, "max_new_tokens", None))
    store = SessionStore(workspace.repo_root + "/.patchfox/sessions")
    ...
    shell_config = resolve_project_shell_config(
        start=workspace.repo_root,
        config_path=getattr(args, "config", None),
        backend=getattr(args, "shell", None),
    )
    shell_backend = resolve_shell_backend(shell_config.backend)
    ...
    if session_id:
        ...
        _attach_provider_controls(agent, args, provider_runtime)
        return agent
    session = None
    if fixed_session_id:
...
def _attach_provider_controls(agent, args, provider_runtime):
    agent.provider_name = provider_runtime.provider_config.name
    agent.provider_warnings = tuple(provider_runtime.provider_config.warnings)
    agent.available_providers = tuple(sorted(load_global_config().get("providers", {})))
    agent.switch_provider = lambda provider, model=None: _switch_agent_provider(
        agent, args, provider, model
    )


def _switch_agent_provider(agent, args, provider, model=None):
    provider_name = str(provider or getattr(agent, "provider_name", "")).strip().lower()
    if not provider_name:
        raise ValueError("provider name cannot be empty")
    global_config = load_global_config()
    profile = global_config.get("providers", {}).get(provider_name, {})
    selected_model = str(model or profile.get("model", "") or "").strip()
    if not selected_model:
        selected_model = str(PROVIDER_DEFAULTS.get(provider_name, {}).get("model", ""))
    if not selected_model:
        raise ValueError(f"model is required for provider {provider_name!r}")

    next_args = copy.copy(args)
    next_args.provider = provider_name
    next_args.model = selected_model
    next_args.max_new_tokens = getattr(args, "_requested_max_new_tokens", None)
    if hasattr(next_args, "_provider_runtime"):
        delattr(next_args, "_provider_runtime")
    runtime = build_provider_runtime(
        next_args, client_classes=_provider_client_classes()
    )
    if not runtime.provider_config.api_key:
        raise ValueError(
            f"provider {provider_name!r} has no API key; run 'patchfox config init'"
        )

    agent.model_client = runtime.model_client
    agent.model_client_router = runtime.model_client_router
    agent.model_client_factory = runtime.model_client_factory
    agent.max_new_tokens = runtime.max_new_tokens
    agent.provider_name = runtime.provider_config.name
    agent.provider_warnings = tuple(runtime.provider_config.warnings)
    agent.available_providers = tuple(sorted(global_config.get("providers", {})))
    agent.session["runtime_identity"] = agent.current_runtime_identity()
    agent.session_event_bus.emit(
        "provider_changed",
        {
            "provider": runtime.provider_config.name,
            "protocol": runtime.provider_config.protocol,
            "model": runtime.provider_config.model,
        },
    )
    agent.refresh_prefix(force=True)
    agent.session_path = agent.session_store.save(agent.session)
    save_project_selection(
        agent.workspace.repo_root,
        runtime.provider_config.name,
        runtime.provider_config.model,
    )
    args.provider = provider_name
    args.model = selected_model
    args._provider_runtime = runtime
    return runtime.provider_config
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"PatchFox {__version__}",
    )
    ...
    parser.add_argument(
        "--repo-root",
        default=None,
        help="Override the repository root used for PatchFox state and relative paths.",
    )
    parser.add_argument(
        "--config", default=None, help="Path to a PatchFox TOML config file."
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    parser.add_argument(
        "--memory-dir",
        default=None,
        help="Memory directory. Defaults to .patchfox/memory in the workspace.",
    )
    parser.add_argument(
        "--no-auto-dream",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        help="Local command shell. On Windows, auto prefers Git Bash, then PowerShell, then CMD.",
    )
    parser.add_argument(
        "--secret-env-name",
...
def handle_repl_command(agent, user_input):
    ...
    if user_input == "/provider" or user_input.startswith("/provider "):
        ...
        if not provider:
            current = getattr(agent, "provider_name", "") or getattr(
                agent.model_client, "provider", "-"
            )
            available = ", ".join(getattr(agent, "available_providers", ())) or "-"
            return True, False, f"provider: {current}\navailable: {available}"
        switch = getattr(agent, "switch_provider", None)
        if not callable(switch):
            return True, False, "error: provider switching is unavailable for this runtime"
        try:
            config = switch(provider)
        except ValueError as exc:
            return True, False, f"error: {exc}"
        return True, False, f"provider: {config.name}\nmodel: {config.model}"
    if user_input == "/model" or user_input.startswith("/model "):
        ...
        switch = getattr(agent, "switch_provider", None)
        if callable(switch):
            provider = getattr(agent, "provider_name", "") or getattr(
                agent.model_client, "provider", ""
            )
            try:
...
def prepare_configuration(args, mode):
    """Ensure interactive launches are configured before constructing an agent."""

    workspace = WorkspaceContext.build(
        args.cwd,
        repo_root_override=getattr(args, "repo_root", None),
    )
    args._project_root = workspace.repo_root
    try:
        missing = configuration_missing(args)
    except ValueError as exc:
        if mode == "one_shot" or getattr(args, "non_interactive", False):
    ...
    if not missing:
        ...
        return tuple(config.warnings)

    if mode == "one_shot" or getattr(args, "non_interactive", False):
        missing_text = ", ".join(missing)
        raise ValueError(
            f"PatchFox configuration is incomplete ({missing_text}). "
            "Run 'patchfox config init' or pass explicit provider credentials."
        )

    confirm_and_migrate_legacy(workspace.repo_root)
    try:
        missing = configuration_missing(args)
    except ValueError:
    ...
    if missing:
        if mode == "tui":
            from .tui.setup import PatchFoxSetupApp

            if not PatchFoxSetupApp().run():
                raise ValueError("PatchFox setup was cancelled")
        else:
            run_text_setup()
    remaining = configuration_missing(args)
    if remaining:
        raise ValueError(
            "PatchFox configuration is still incomplete: " + ", ".join(remaining)
        )
    return ()


def _run_config_command(argv):
    parser = argparse.ArgumentParser(prog="patchfox config")
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("init", help="Create or update global provider configuration.")
    show_parser = subparsers.add_parser(
        "show", help="Show the effective configuration with credentials redacted."
    )
    show_parser.add_argument("--cwd", default=".", help="Project directory to resolve.")
    args = parser.parse_args(argv)
    try:
        if args.command == "init":
            run_text_setup()
        else:
            project_root = WorkspaceContext.build(args.cwd).repo_root
            print(render_config_show(project_root))
...
def main(argv=None):
    ...
    if raw_argv and raw_argv[0] == "config":
        return _run_config_command(raw_argv[1:])
    ...
    try:
        warnings = prepare_configuration(args, mode)
    ...
    if mode == "tui":
        from .tui.app import PatchFoxTuiApp
