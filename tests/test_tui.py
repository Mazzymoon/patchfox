...
def assistant_contents(app):
    from patchfox.tui.widgets import AssistantMessage
...
async def wait_for_tool_card_status(app, pilot, status, attempts=40, delay=0.1):
    from patchfox.tui.widgets import ToolCard
...
def test_cli_defaults_interactive_tty_mode_to_tui(monkeypatch):
    from patchfox.cli import build_arg_parser, interaction_mode

    monkeypatch.setattr(
        "patchfox.cli.sys.stdin", type("Stdin", (), {"isatty": lambda self: True})()
    )
...
def test_cli_uses_plain_repl_for_piped_stdin(monkeypatch):
    ...
    monkeypatch.setattr(
        "patchfox.cli.sys.stdin", type("Stdin", (), {"isatty": lambda self: False})()
    )
    args = build_arg_parser().parse_args(["--cwd", "/tmp/workspace"])
...
def test_status_bar_shows_runtime_identity(tmp_path):
    from patchfox.tui.widgets import StatusBar
...
def test_status_bar_reads_context_usage_governance_fields():
    ...
    assert "ctx:1.2k/200k" in rendered_text(status)
...
def test_status_bar_optionally_shows_context_pressure_fields():
    ...
    text = rendered_text(status)
    assert "ctx:~500/1k" in text
    assert "pressure:medium" in text
...
def test_cli_plan_mode_and_session_commands_expose_runtime_state(tmp_path):
    from patchfox.cli import handle_repl_command
    ...
    assert ".patchfox/plans/refactor-auth-plan.md" in output
    assert agent.runtime_mode == "plan"

    handled, _, output = handle_repl_command(agent, "/mode")
    assert handled is True
    assert "runtime mode: plan" in output
    assert "plan path: .patchfox/plans/refactor-auth-plan.md" in output
...
def test_slash_command_registry_suggests_and_parses_subagent():
    from patchfox.commands.slash import (
        parse_subagent_args,
        resolve_command,
        suggest_commands,
    )
...
def test_slash_help_uses_markdown_list_rows():
    from patchfox.commands.slash import command_help_text
...
async def test_tui_slash_suggestions_complete_partial_command(tmp_path):
    ...
    from patchfox.tui.widgets import InputBar, SlashSuggestions

    app = PatchFoxTuiApp(build_agent(tmp_path, []))
...
async def test_tui_hides_welcome_after_first_turn_so_chat_stays_visible(tmp_path):
    from patchfox.tui.app import PatchFoxTuiApp
    from patchfox.tui.widgets import ChatLog, InputBar, WelcomeBanner
...
async def test_tui_chat_stream_uses_terminal_transcript_layout(tmp_path):
    from patchfox.tui.app import PatchFoxTuiApp
    from patchfox.tui.widgets import AssistantMessage, ChatLog, InputBar, UserMessage
    ...
    async with app.run_test(size=(100, 20)) as pilot:
        ...
        bar.input.value = "你是谁"
        await pilot.press("enter")
        text = await wait_for_assistant(app, pilot, "我是 patchfox。")
...
async def test_tui_renders_tool_card_result(tmp_path):
    from patchfox.tui.app import PatchFoxTuiApp
    from patchfox.tui.widgets import InputBar, ToolCard
...
async def test_tui_approval_prompt_controls_risky_tool(tmp_path):
    from patchfox.tui.app import PatchFoxTuiApp
    from patchfox.tui.widgets import ConfirmPrompt, InputBar
...
async def test_tui_ask_user_prompt_returns_selected_choice(tmp_path):
    from patchfox.tui.app import PatchFoxTuiApp
    from patchfox.tui.widgets import AskUserPrompt, InputBar
