...
info()    { printf "${CYAN}[patchfox]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[patchfox]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[patchfox]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[patchfox] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
    printf "\n${BOLD}╔══════════════════════════════════════════╗${RESET}\n"
    printf   "${BOLD}║        patchfox  一键安装                    ║${RESET}\n"
    printf   "${BOLD}╚══════════════════════════════════════════╝${RESET}\n\n"
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    if [[ -d "$INSTALL_DIR/.git" ]]; then
        ...
        info "克隆 patchfox 到 ${INSTALL_DIR} ..."
        rm -rf "$INSTALL_DIR"
    ...
    BIN_DIR="${PATCHFOX_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/patchfox"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/patchfox" "\$@"
EOF
    chmod +x "$LAUNCHER"

    printf "\n"
    success "patchfox 安装完成！"
    printf "\n"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -qx "$BIN_DIR"; then
        ...
        printf "  运行：${BOLD}patchfox${RESET}\n\n"
    fi

    printf "  首次配置：${BOLD}patchfox config init${RESET}\n"
    printf "  配置会保存在 ${CYAN}~/.patchfox${RESET}，无需为每个项目设置环境变量。\n\n"
