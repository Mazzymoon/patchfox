...
from patchfox.evaluation.metrics import (
    _provider_profile,
...
def test_provider_profile_ignores_unsafe_project_profile_and_uses_process_env(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    with patch.dict(
        os.environ,
        {
            "PATCHFOX_DEEPSEEK_API_KEY": "sk-legacy-deepseek",
            "PATCHFOX_DEEPSEEK_MODEL": "legacy-deepseek-model",
            "PATCHFOX_DEEPSEEK_API_BASE": "https://legacy.deepseek.example/anthropic",
        },
        clear=True,
    ):
    ...
    assert profile["model"] == "legacy-deepseek-model"
    assert profile["base_url"] == "https://legacy.deepseek.example/anthropic"
...
def test_write_benchmark_core_report_marks_resume_safe_metrics(tmp_path):
    ...
    report_path = tmp_path / "docs" / "metrics" / "patchfox-benchmark-core-report.md"
...
def test_write_benchmark_core_report_includes_optional_context_ab(tmp_path):
    from patchfox.evaluation.context_cost import run_deterministic_prompt_experiment, write_experiment_artifacts
